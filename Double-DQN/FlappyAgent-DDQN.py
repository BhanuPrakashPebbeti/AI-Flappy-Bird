...
class Agent:
    ...
    def __init__(self, gamma, lr, input_dims, batch_size, n_actions, tau=0.8, replay_mem_size=1000, epsilon=1.0,
        ...
        if pretrained:
            self.Q_eval.load_state_dict(torch.load(os.path.realpath(os.path.join(ROOT_DIR, "FLAPPY-BIRD-DDQN.pt"))))
            self.Q_target.load_state_dict(
                torch.load(os.path.realpath(os.path.join(ROOT_DIR, "FLAPPY-BIRD-TARGET-DDQN.pt"))))
    ...
    def learn(self):
        ...
        q_eval_next = self.Q_eval.forward(states_)
        max_actions = torch.argmax(q_eval_next, dim = 1)
        q_target = rewards + self.gamma * q_next[indices, max_actions].detach()
        q_target[dones] = 0
...
for episode in range(START, EPISODES):
    ...
    if episode % 2 == 0:
        ...
        torch.save(agent.Q_eval.state_dict(), os.path.realpath(os.path.join(ROOT_DIR, "FLAPPY-BIRD-DDQN.pt")))
        torch.save(agent.Q_target.state_dict(), os.path.realpath(os.path.join(ROOT_DIR, "FLAPPY-BIRD-TARGET-DDQN.pt")))
